...
#
# 设计依据：docs/design/CI冒烟门禁设计.md §4.1 + §6.2 + 附录 A
#
# 提供共享函数：
...
#   experiments/ci/lib_smoke.ps1 -> repo root
...
function Resolve-ProjectPaths {
    <#
        .SYNOPSIS
        返回项目根、construct-rs 子目录、venv python.exe 等路径。

        .DESCRIPTION
        venv 路径来自 docs/design/CI冒烟门禁设计.md §4.1 / §5.3 约定：
          crs_venv_new   -> construct-rs maturin develop 目标 venv
          crs_venv_py_new -> Python construct 对比 venv（L3 用，L1 不需要）
        位于 %TEMP%/opencode/ 下。
    #>
    $root = Get-ProjectRoot
    $crsDir = Join-Path $root "construct-rs"
    ...
    return @{
        ...
        ReportsDir      = Join-Path $root "experiments\ci\reports"
    }
}
