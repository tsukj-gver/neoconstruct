...
设计依据：
- docs/design/CI冒烟门禁设计.md §4.4（L4 核查清单）
- docs/agents/auditor-extension.md §第 7 类审计项（10 项 check 规范来源）
...
# run_l4_consistency.py 位于 <root>/experiments/ci/
...
def _parse_pointer(pointer: str) -> List[Tuple[str, List[int]]]:
    ...
    格式：``path:line`` 或 ``path:line-line``，多个用 ``;`` 分隔。

    返回 [(file_path, [line_numbers]), ...]。文件不存在的条目也会返回，
    供调用方按行号定位核查。
    """
    ...
    for part in parts:
        # 兼容 "MEMORY.md:109" / "plans/xxx.md:60-63" / "construct-rs/src/.../x.rs:1"
        m = re.match(r"^(.+):(\d+)(?:-(\d+))?$", part)
        if not m:
            # 不识别格式：作为 evidence 报告
            results.append((part, []))
            continue
        file_path = m.group(1)
        start = int(m.group(2))
...
def check_c5_inventory_perf_data_source_reachable() -> CheckResult:
    ...
    perf_data_source 字段格式：``plans/xxx.md:60-63;plans/yyy.md:2242-2265``
    """
    evidence: List[str] = []
    _, rows, _ = _parse_csv_with_header(
        INVENTORY_CSV, INVENTORY_EXPECTED_COLUMNS
    ...
    for row in rows:
        ...
        for file_path, lines in pointers:
            if not lines:
                # 格式不识别
                evidence.append(
                    f"{name}: cannot parse pointer: {file_path}"
                )
                continue
...
def check_c6_perf_scenarios_data_source_reachable() -> CheckResult:
    ...
    data_source 字段格式：``plans/xxx.md:5005`` 或多个用 ``;`` 分隔。
    """
    evidence: List[str] = []
    _, rows, _ = _parse_csv_with_header(
        PERF_SCENARIOS_CSV, PERF_SCENARIOS_EXPECTED_COLUMNS
        ...
        for file_path, lines in pointers:
            if not lines:
                evidence.append(
                    ...
                    f"cannot parse pointer: {file_path}"
                )
                continue
            full = PROJECT_ROOT / file_path
            if not full.exists():
