...
> **META-CI** 交付物。设计依据：[`docs/design/CI冒烟门禁设计.md`](../../docs/design/CI冒烟门禁设计.md)。
...
```powershell
powershell -File experiments/ci/run_smoke.ps1 -Level All
```
...
```powershell
powershell -File experiments/ci/run_smoke.ps1 -Level "L1,L4"
```
...
**位置**：`experiments/ci/known_exemptions.json`（PM 主维护）
...
所有报告输出到 `experiments/ci/reports/`（`.gitignore` 排除）：
...
- 设计：[`docs/design/CI冒烟门禁设计.md`](../../docs/design/CI冒烟门禁设计.md)（§0-§8 + 附录 A/B，4 层门禁完整规范）
- L-09 教训：`experiences.md §L-09`（跨时段性能对比消除法归因失效）
