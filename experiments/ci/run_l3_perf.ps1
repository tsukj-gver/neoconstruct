...
#
# 设计依据：docs/design/CI冒烟门禁设计.md §5 + §6.3（子任务 1c 范围）
#
# 职责：
...
param(
    ...
    [switch]$NoAbTest
    ,
    # 报告输出路径（默认 experiments/ci/reports/<ts>_L3_report.json）
    [string]$ReportPath
    ,
...
function Assert-L3Preconditions {
    if (-not (Test-Path -LiteralPath $CrsPython)) {
        ...
        Write-CiLog "Hint: experiments\ci\run_smoke.ps1 -Level L1" -Level WARN
        exit 2
    }
