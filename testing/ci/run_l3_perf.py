...
设计依据：
- docs/design/基础设施/CI冒烟门禁设计.md §5（L3 性能回归检测框架）
- harness/experiences.md §L-09（跨时段性能对比消除法归因失效）
...
# 项目路径定位
# ---------------------------------------------------------------------
# run_l3_perf.py 位于 <root>/testing/ci/
PROJECT_ROOT = Path(__file__).resolve().parents[2]
_SCRIPT_DIR = Path(__file__).resolve().parent
DEFAULT_BASELINE = PROJECT_ROOT / "docs" / "perf-scenarios.csv"
DEFAULT_EXEMPTIONS = _SCRIPT_DIR / "known_exemptions.json"
...
def run_ab_test_upgrade(
    ...
    triggered: List[str] = []
    reports: List[str] = []
    ab_harness = _SCRIPT_DIR / "ab_test" / "ab_harness.ps1"
