...
//!
//! Python 内部类（`construct._internals`）：`EnumInteger(int)` 是 int 子类（无映射 fallback），
...
mod tests {
    ...
    fn make_enum_node(py: Python<'_>) -> Option<EnumNode> {
        // 加载 EnumInteger 类（从 construct._internals，若可用）。
    ...
    fn parse_mapped_returns_label() {
        ...
        with_py(|py| {
            let node = match make_enum_node(py) {
                Some(n) => n,
                None => {
                    eprintln!("skipping: construct._internals.EnumInteger not available");
                    return;
                }
            };
