...
//! 两者都从 `construct.lib.hex` 模块编译期加载。
...
/// 编译期从 `construct.lib.hex` 模块加载 Hex 3 个显示类。
///
/// 模块加载失败（理论不应发生，`construct.lib.hex` 是核心库）→ CompilationError。
pub fn load_hex_display_classes(py: Python<'_>) -> Result<HexDisplayClasses, ConstructError> {
    let module = py
        .import_bound("construct.lib.hex")
        .map_err(|e| ConstructError::Compilation {
            message: format!("failed to import construct.lib.hex: {}", e),
        })?;
    let get = |name: &str| -> Result<Py<PyType>, ConstructError> {
        module
            .getattr(name)
            .map_err(|e| ConstructError::Compilation {
                message: format!("failed to get construct.lib.hex.{}: {}", name, e),
            })?
            .extract::<Py<PyType>>()
            .map_err(|e| ConstructError::Compilation {
                message: format!("construct.lib.hex.{} is not a Python type: {}", name, e),
            })
    };
...
/// 编译期从 `construct.lib.hex` 模块加载 HexDump 2 个显示类。
...
mod tests {
    ...
    /// 加载真实 Hex 显示类（如果 construct.lib.hex 可用）。
    ...
    fn parse_int_returns_hex_displayed_integer() {
        ...
        with_py(|py| {
            let classes = match try_load_classes(py) {
                Some(c) => c,
                None => {
                    eprintln!("skipping: construct.lib.hex not available");
                    return;
                }
            };
    ...
    fn node_display_cls<'py>(py: Python<'py>, name: &str) -> Bound<'py, PyType> {
        py.import_bound("construct.lib.hex")
            .expect("import")
            .getattr(name)
            .expect("getattr")
