...
pub struct BinExpr {
    ...
    pub left: Box<CombinedExpr>,
    /// The right-hand side expression.
    pub right: Box<CombinedExpr>,
}
...
impl BinExpr {
    ...
    /// );
    /// ```
    pub fn new<L: Into<CombinedExpr>, R: Into<CombinedExpr>>(op: BinOp, left: L, right: R) -> Self {
        BinExpr {
            op,
...
pub struct UniExpr {
    ...
    pub inner: Box<CombinedExpr>,
}

impl UniExpr {
    /// Creates a new `UniExpr` with the given operator and inner expression.
    pub fn new<E: Into<CombinedExpr>>(op: UniOp, inner: E) -> Self {
        UniExpr {
            op,
...
impl FuncPath {
    ...
    pub fn new<E: Into<CombinedExpr>>(name: impl Into<String>, inner: E) -> Self {
        FuncPath {
            name: name.into(),
            inner: Box::new(inner.into()),
        }
...
impl CondExpr {
    ...
    pub fn new<C: Into<CombinedExpr>, T: Into<CombinedExpr>, E: Into<CombinedExpr>>(
        ...
        CondExpr {
            ...
            then_val: Box::new(then_val.into()),
            else_val: Box::new(else_val.into()),
        }
    }
}
...
pub struct ListPath {
    ...
    pub inner: Option<Box<CombinedExpr>>,
}
...
impl ListPath {
    ...
    #[must_use]
    pub fn new_indexed<E: Into<CombinedExpr>>(index: i64, inner: E) -> Self {
        ListPath {
            index,
...
// ===========================================================================
// DynamicExpr — escape hatch for non-variant expression types (e.g. PyClosure)
// ===========================================================================

/// Wrapper for `dyn Evaluate` trait objects, enabling foreign expression types
/// (such as PyO3-backed closures) to be stored inside [`CombinedExpr`].
///
/// This mirrors [`CombinedConstruct::Dynamic`](crate::combined::CombinedConstruct::Dynamic).
/// Stored behind `Arc<dyn Evaluate>` so the variant is cloneable.
#[derive(Clone)]
pub struct DynamicExpr {
    inner: std::sync::Arc<dyn Evaluate>,
}

impl DynamicExpr {
    /// Creates a new [`DynamicExpr`] wrapping the given expression.
    #[must_use]
    pub fn new<E: Evaluate + 'static>(expr: E) -> Self {
        DynamicExpr {
            inner: std::sync::Arc::new(expr),
        }
    }

    /// Creates a [`DynamicExpr`] from an existing `Arc<dyn Evaluate>`.
    #[must_use]
    pub fn from_arc(arc: std::sync::Arc<dyn Evaluate>) -> Self {
        DynamicExpr { inner: arc }
    }
}

impl Evaluate for DynamicExpr {
    fn evaluate(&self, ctx: &Context, obj: Option<&Value>) -> Result<Value> {
        self.inner.evaluate(ctx, obj)
    }
}

impl std::fmt::Debug for DynamicExpr {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DynamicExpr")
            .field("inner", &"<dyn Evaluate>")
            .finish()
    }
}

/// Wraps any `Evaluate` into a [`CombinedExpr::Dynamic`] variant.
///
/// Use this for foreign expression types (e.g. PyClosure) that are not
/// named variants of [`CombinedExpr`]. For built-in expressions, prefer
/// `.into()` which uses static dispatch.
#[must_use]
pub fn dynamic_expr<E: Evaluate + 'static>(expr: E) -> CombinedExpr {
    CombinedExpr::Dynamic(DynamicExpr::new(expr))
}
...
/// enum_dispatch wrapper enum that replaces `Box<dyn Evaluate>`.
///
/// All 7 expression node types are variants, plus a [`Dynamic`] variant for
/// foreign expression types. [`Evaluate`] methods are dispatched via a `match`
/// generated by `enum_dispatch`.
///
/// [`Dynamic`]: CombinedExpr::Dynamic
#[derive(Clone, Debug)]
...
pub enum CombinedExpr {
    ...
    /// Wraps [`ListPath`].
    ListPath(ListPath),
    /// Wraps [`DynamicExpr`] — escape hatch for foreign expression types.
    Dynamic(DynamicExpr),
}
...
/// existing `Box<CombinedExpr>` interchangeably.
///
/// # Example
///
/// ```
...
impl IntoExpr for usize {
    fn into_expr(self) -> Box<CombinedExpr> {
        Box::new(CombinedExpr::ConstExpr(ConstExpr::new(Value::UInt(
            self as u64,
        ))))
    }
}
...
impl IntoExpr for i64 {
    fn into_expr(self) -> Box<CombinedExpr> {
        Box::new(CombinedExpr::ConstExpr(ConstExpr::new(Value::Int(self))))
    }
}

impl IntoExpr for u64 {
    fn into_expr(self) -> Box<CombinedExpr> {
        Box::new(CombinedExpr::ConstExpr(ConstExpr::new(Value::UInt(self))))
    }
}

impl IntoExpr for Path {
    fn into_expr(self) -> Box<CombinedExpr> {
        Box::new(CombinedExpr::Path(self))
    }
}

impl IntoExpr for ListPath {
    fn into_expr(self) -> Box<CombinedExpr> {
        Box::new(CombinedExpr::ListPath(self))
    }
}
...
impl Path {
    ...
    pub fn add<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::Add, self, rhs)
    }

    /// Returns a [`BinExpr`] that subtracts `rhs` from this path.
    pub fn sub<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::Sub, self, rhs)
    }

    /// Returns a [`BinExpr`] that multiplies this path by `rhs`.
    pub fn mul<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::Mul, self, rhs)
    }

    /// Returns a [`BinExpr`] that divides this path by `rhs`.
    pub fn div<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::Div, self, rhs)
    }

    /// Returns a [`BinExpr`] that computes `this % rhs`.
    pub fn rem<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::Mod, self, rhs)
    }

    /// Returns a [`BinExpr`] that computes `this << rhs`.
    pub fn shl<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::Shl, self, rhs)
    }

    /// Returns a [`BinExpr`] that computes `this >> rhs`.
    pub fn shr<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::Shr, self, rhs)
    }

    /// Returns a [`BinExpr`] that computes `this & rhs`.
    pub fn bitand<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::BitAnd, self, rhs)
    }

    /// Returns a [`BinExpr`] that computes `this | rhs`.
    pub fn bitor<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::BitOr, self, rhs)
    }

    /// Returns a [`BinExpr`] that computes `this ^ rhs`.
    pub fn bitxor<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::BitXor, self, rhs)
    }

    /// Returns a [`BinExpr`] that computes `this == rhs`.
    pub fn eq_expr<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::Eq, self, rhs)
    }

    /// Returns a [`BinExpr`] that computes `this != rhs`.
    pub fn ne_expr<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::Ne, self, rhs)
    }

    /// Returns a [`BinExpr`] that computes `this < rhs`.
    pub fn lt<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::Lt, self, rhs)
    }

    /// Returns a [`BinExpr`] that computes `this > rhs`.
    pub fn gt<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::Gt, self, rhs)
    }

    /// Returns a [`BinExpr`] that computes `this <= rhs`.
    pub fn le<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
        BinExpr::new(BinOp::Le, self, rhs)
    }

    /// Returns a [`BinExpr`] that computes `this >= rhs`.
    pub fn ge<E: Into<CombinedExpr>>(self, rhs: E) -> BinExpr {
...
mod tests {
    ...
    #[test]
    fn into_expr_usize() {
        let expr: Box<CombinedExpr> = 10usize.into_expr();
        let result = expr.evaluate(&Context::new(), None).unwrap();
    ...
    #[test]
    fn into_expr_i64() {
        let expr: Box<CombinedExpr> = (-42i64).into_expr();
        let result = expr.evaluate(&Context::new(), None).unwrap();
        assert_eq!(result, Value::Int(-42));
    }
    ...
    #[test]
    fn into_expr_u64() {
        let expr: Box<CombinedExpr> = 100u64.into_expr();
        let result = expr.evaluate(&Context::new(), None).unwrap();
    ...
    fn into_expr_path() {
        ...
        let expr: Box<CombinedExpr> = path.into_expr();
        let result = expr.evaluate(&ctx, None).unwrap();
        assert_eq!(result, Value::Int(7));
    }
    ...
    #[test]
    fn into_expr_boxed() {
        let boxed: Box<CombinedExpr> = Box::new(ConstExpr::new(Value::Int(99)).into());
        let expr: Box<CombinedExpr> = boxed.into_expr();
        let result = expr.evaluate(&Context::new(), None).unwrap();
        assert_eq!(result, Value::Int(99));
    }
