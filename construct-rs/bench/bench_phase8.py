...
# ---------------------------------------------------------------------------
# venv 解析（与 bench_phase6/7.py 一致）
# ---------------------------------------------------------------------------
# 8.ENV（2026-08-06）：切换至 Python 3.13 非 abi3 venv（pyo3 0.22 原生支持 3.13，
# 不再依赖 PYO3_USE_ABI3_FORWARD_COMPATIBILITY）。旧 crs_venv_new / crs_venv_py_new
# （Python 3.14 + abi3）保留为 legacy 基线对照。
