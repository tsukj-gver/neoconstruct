...
## 分派标准禁令段（每次分派 prompt 必含，禁止省略）

```
- 遵守 AGENTS.md §2 权限与 §3 红线；交付物/探针放工作区合规路径（探针：experiments/<语义名>-probe/）
- 遇权限拦截：停止并上报，禁止 bash 绕过（L-15）
- 不做 git 操作（commit/tag 由 PM 执行）
- 单次写入 ≤300 行
```

（PM 职责纪律：commit 前核对 `git status` 明细，禁止 add -A 盲提交；被中断任务的产物是进行中工作，由任务所有者接手处置，PM 不代删/代改。）
