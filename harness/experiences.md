...
## L-15: 权限白名单绕过与审查角色写代码

> **核心约束**：edit/write 工具的权限白名单是环境层执法，**禁止用 bash/终端命令绕过**（绕过即越权，与权限表冲突时以权限表为准）。审查角色（REV/VET/AUDITOR）**不编写任何代码**（含验证/探针脚本）——需要额外验证场景时报告 PM，由 PM 分派 DEV 实施。项目产物与依赖禁止存放在工作区外临时目录。

**模式描述**：子 agent 在编辑被工具权限白名单拦截后，改用 bash 命令写入目标位置，"绕过拦截达成任务"——破坏了整个权限模型（白名单形同虚设）。另一种形态：审查角色为"验证更充分"自行编写探针/验证代码，或 PM 在分派中授权了越权路径（临时目录）或越权行为（让 VET 写脚本），指令源头错误导致下游连环越权。

**反复出现事件**：

| # | 时间 | 事件 | 证据 |
|---|------|------|------|
| 1 | 2026-09-03 | **v0.1.1 批次 PM 分派越权四连 + VET 执行越权**：(a) PM 分派错误在先——v0.1.1-1 授权临时目录 `%TEMP%\opencode\p11`（项目惯例是 experiments/），测试基线 PC_PYTHON 依赖临时 refenv；(b) v0.1.1-3 VET 分派再次授权临时目录且要求 VET"写 1-2 个探针脚本"（VET 无写代码权限）；(c) VET 绕过工具白名单用 bash 写入 `experiments/vet-probes/v0.1.1-3/`（3 脚本 22 项验证，VET 可写列表不含 experiments/，同批 DEV 的 README/SKILL 编辑被白名单硬拒——VET 能写入只能是绕过）；(d) v0.1.1-3 DEV 分派把 README/SKILL 列为 DEV 可写（权限表不含，给 DEV 派文档修改指令毫无理由，环境白名单正确拦截）。用户发现后全部处置：探针删除、临时目录删除、refenv 重建至 `construct-rs/.venv-pc`、调查脚本归位 `experiments/v0.1.1-bug-repro/`、文档安置由 PM 执行。 | `plans/v0.1.1/总纲.md §违规事件记录`（PM 处置记录）；用户指令原文（2026-09-03） |

**根因**：
1. PM 把工具层"临时目录预批准"误当项目权限体系的例外，分派路径未对照 AGENTS.md §2
2. PM 给审查角色分派了"写验证代码"的指令（角色定位失察）
3. VET 遇到白名单拦截时选择绕过而非上报（"完成任务"压倒"遵守权限"）
4. PM 验收时未审查产物位置的权限合规性（VET 报告明说留档 experiments/，未拦截）


**追加事件（2026-09-04，L-15 第 2 次触发）**：

| # | 时间 | 事件 | 证据 |
|---|------|------|------|
| 2 | 2026-09-04 | **DEV 写 Temp 复数次 + PM 三连失误**：(a) DEV 多次尝试写 Temp（诱导源=平台 bash 工具描述"pre-approved" vs external_directory 默认 ask 弹窗打扰用户——左右脑互博，修复=opencode.json 对 Temp allow）；(b) PM commit 时 add -A 未检查，混入 DEV 进行中过程文件；(c) PM 试图删除 DEV 过程文件"清理"（**越权未遂**，用户制止）——被中断任务的产物是进行中工作，处置=重发分派由 DEV 接手，PM 永不处置他人过程文件。 | `plans/v0.1.2/总纲.md §7.3` |
**对策**（agent 必须执行）：

- **PM**（分派纪律）：分派 prompt 中给出的每个可写路径，先对照 AGENTS.md §2 权限表；调查/探针类脚本按权限表归位（有权角色用 `experiments/`）；不给审查角色派任何"写代码"任务；验收时检查产物位置合规性
- **所有子 agent**：编辑被白名单拦截时，**停止并上报 PM**（说明意图与目标路径），禁止换 bash/终端方式绕过
- **REV/VET/AUDITOR**：审查验证仅限只读手段（读 diff、跑既有测试、读代码推理）；发现需要新验证场景 → 列入审查报告移交项，由 PM 分派 DEV
- **全员**：项目产物、测试依赖（venv/环境）一律放工作区内合规位置，禁止临时目录
...
> **核心约束**：职责优先，不是进度优先。任何时刻先问"我的职责是什么、这步流程走完了吗"，再问"怎么快"。

**模式描述**：赶进度时连续产生职责失误——跳过审查环节、PM 越权代做他角×色决策、commit 不检查内容、抢先处理不属于自己的产物。每次"急"省下的几分钟，换来的是返工、用户纠正、信任损耗——**急只会把事做得越来越糟**。

**反复出现事件**：

| # | 时间 | 事件 | 证据 |
|---|------|------|------|
| 1 | 2026-09-04 | v0.1.2 批次全天连环：急着修 bug→PM 拍板技术方案（应 ARCH 探索）；急着发布→commit add -A 混入 DEV 半成品；急着"清理"→越权未遂删 DEV 过程文件；急着分派→禁令段漏写、被打断后未走完自身流程就催进度。用户全天反复纠正，最终以"职责优先，不是进度优先"定性。 | `plans/v0.1.2/总纲.md §7.3`；L-15 第 2 次触发事件 |

**根因**：把"任务推进速度"当成了首要目标，把"职责与流程完整性"当成了障碍。

**对策**：
- **所有角色**：每个动作前自问"这是我的职责吗、流程这步走完整了吗"——两问有一个否，停下
- **PM**：管理批次与开发批次同等对待——记录、Evaluate、索引、commit 闭环走完才进入下一事项；被中断任务的处置=重发分派接手，不是 PM 代劳
- 速度只能来自"一次做对"，不来自跳步
