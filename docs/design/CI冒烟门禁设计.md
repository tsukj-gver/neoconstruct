...
| 性能回归方法学 | `experiences.md §L-09` + `performance-gate/SKILL.md Checkpoint 4` | §4 L3 + §5 |
...
| 一致性核查清单 | `docs/agents/auditor-extension.md §第 7 类审计项` | §4 L4 |
...
- **默认不安装**：提供 `experiments/ci/install_hook.ps1`，开发者主动选择是否启用（避免"hook 太慢被绕过"反模式）
...
**规范来源**：`docs/agents/auditor-extension.md §第 7 类审计项`（AUDITOR 在 phase 验收时审；L4 把它日常化）

**执行脚本**：`experiments/ci/run_l4_consistency.py`（Python，纯静态检查，不需要跑 bench）
...
> 本章是 META-CI-1 的核心。设计严格对齐 `experiences.md §L-09` + `performance-gate/SKILL.md Checkpoint 4`，
...
**规范来源**：`experiences.md §L-09` + `performance-gate/SKILL.md Checkpoint 4` 结论判据表
...
- `experiments/ci/reports/known_exemptions.json`：已知豁免清单（**初始含 21 条目**，对齐用户决策事项 A/B/C，按 META-CI-1-REV 关键缺陷修订）
...
- 负面：需维护 `experiments/ci/` 代码（~5-8 个脚本）；L3 单次跑 10-20min
...
### A.3 `experiments/ci/run_l4_consistency.py`
