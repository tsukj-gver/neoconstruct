...
参考实现：Python `construct/`（源码位置速查见 `.opencode/agents/architect.md §Python 参考速查`）

对照方法：
- [ ] parse 一致性：相同二进制 → 相同解析结果
...
补充 base §API 一致性：
- [ ] snake_case 命名风格统一
- [ ] 与设计文档（`docs/design/模块设计-*.md`）签名一致
...
- `plans/phaseN/过程记录.md`（审查结果，按 `docs/文档元数据规范.md` 格式）
- `experiments/**`
