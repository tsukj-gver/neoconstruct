...
> （详见 `docs/design/基础设施/架构设计.md` §G.6 性能模型修正）。
...
### 4.7 实测确认（2026-06-22，P0 优化后）

> **R3 预测已被实测大幅超越。** P0 优化（fat LTO + Path/Context/map_err 移除）实施后，
> 方案 B' 的实测性能远超 §4.4 的 R3 预测。以下表保留 R3 预测原文，并补充实测确认值。

| 用例 | N | §4.4 R3 预测加速比 | **P0 实测加速比** | §4.4 R3 预测 Full (ns) | **P0 实测 Full (ns)** |
|------|---|-------------------|-----------------|----------------------|---------------------|
| B1 | 3 | ~7.3x | **8.09x** ✓ | ~425 ns | **386 ns** |
| B2 | 10 | ~6.8x | **9.66x** ✓ | ~880 ns | **628 ns** |
| B3 | 50 | ~10.3x | **11.54x** ✓ | ~4230 ns | **2020 ns** |
| B4 | 100 | ~9.0x | **12.06x** ✓ | ~9230 ns | **3656 ns** |

> **偏差根因**：R3 per-field 预测 ~65-90ns，实测仅 ~33ns（线性回归：固定 ~229ns +
> per-field ~33ns）。偏差来自 R3 模型未计入 pyo3 抽象层的隐性开销（无 LTO 时
> ~140ns/field），P0-1 启用 fat LTO 后此开销被内联消除。
>
> **完整实测数据见 `docs/analysis/perf-p0-analysis.md` §8.2**；完整偏差分析见同文档
> §8-§9；修正后的性能模型见 `docs/design/基础设施/架构设计.md` §G.6。
...
*本设计修订（**R3，2026-06-22**）经 PM 确认后，由 ARCH 更新 `docs/design/基础设施/架构设计.md`
