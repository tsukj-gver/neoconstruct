"""Pure-Python :class:`Container` / :class:`ListContainer` ported line-by-line
from the upstream ``construct.lib.containers`` module.

This module mirrors the original 290-line implementation so that all behaviours
-- including the ``self.__dict__ = self`` trick, method shadowing, recursion
locked pretty-printing, and regex searching -- are reproduced bit-for-bit.

Per Phase 10 sub-task 10.3 design decisions, the ``from construct.lib.py3compat
import *`` line was dropped because none of its symbols are actually used here.
"""
