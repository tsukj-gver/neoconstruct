"""Pure-Python expression system ported line-by-line from ``construct.expr``.

This module is a verbatim port of the upstream ``construct/construct/expr.py``
(256 lines). It provides the declarative expression objects used to reference
context fields (``this``, ``obj_``, ``list_``), apply arithmetic/logical
operators (``BinExpr``, ``UniExpr``), and invoke built-in functions
(``len_``, ``sum_``, ``min_``, ``max_``, ``abs_``).

All 28 operator overloads (14 binary + 14 reflected), 3 unary operators,
6 comparison operators, and the ``__contains__`` method are reproduced
bit-for-bit from the original. The ``evaluate()`` helper is also provided
here (rather than in a separate ``core`` module) per Phase 10 sub-task 10.4
design decision.
"""
...
class ExprMixin(object):
    """Mixin providing operator overloads for all expression classes.

    Each binary operator returns a new :class:`BinExpr`; each unary operator
    returns a new :class:`UniExpr`. Comparison operators also return
    ``BinExpr`` objects (deferred evaluation) rather than ``bool``.
    """
...
class Path(ExprMixin):
    """Attribute/subscript path expression (``this`` / ``obj_``)."""
...
class Path2(ExprMixin):
    """List-index path expression (``list_``)."""
...
class FuncPath(ExprMixin):
    """Built-in function path expression (``len_`` / ``sum_`` / etc.)."""
...
def evaluate(param, context):
    """Evaluate a parameter that may be a callable expression or a plain value.

    If ``param`` is callable (Path, BinExpr, FuncPath, lambda, etc.), it is
    called with ``context`` as the single argument. Otherwise ``param`` is
    returned as-is.

    This mirrors ``construct.core.evaluate`` and is used by pure-Python
    adapter base classes and other constructs that accept expression
    parameters.
    """
