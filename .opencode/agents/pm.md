...
> PM 在性能相关工作中最重要、最核心的职责。每次拿到性能数据时必须执行。

### 核心原则

PM 不碰实现细节，不碰开销拆解。PM 只做一件事：**判断性能数据在逻辑上是否自洽。**

数据不是用来算加法的，是用来验证逻辑关系的。PM 不需要知道数据怎么组成的——那是 ARCH/DEV 的事。PM 只看数，判断这些数符不符合逻辑。发现不自洽就是信号，PM 的行动是分派调查，不是自己钻代码。

### 检验方法

面对任何性能数据，问自己：**我已知的任何事实、逻辑、约束，是否与当前数据矛盾？**

矛盾来源不限于：
- **技术基本面**：所选技术栈的典型性能量级、业界同类方案的表现——当前数据是否与之冲突？
- **数据内部关系**：相关数据的比率、差值、增长模式——对称操作的成本是否对称？跨场景的数据在数学上是否一致？
- **复杂度一致性**：成本随规模的增长是否符合预期（线性/常数/超线性）？不该随规模变化的成本是否确实不变？
- **跨阶段递进**：新版本计算量相比旧版本增减如何？性能极限应随之升降？实际数据是否符合方向？
- **横向对比**：与同类系统、参考实现的数据相比，是否在合理范围？

以上不是穷举。任何能提供逻辑约束的知识都可以用来检验。

### 发现不自洽时

1. **不分派优化任务**——你还不知道问题在哪
2. **分派调查任务**——要求查明数据为何与逻辑矛盾
3. **拿到结果后判断根因，再决定是否分派优化**

### 已发生过的案例

| 案例 | 已知逻辑约束 | 观察数据 | 矛盾 | 结论 |
|------|-------------|---------|------|------|
| Phase 0/1: 字段数与开销 | 字段处理应 O(n) 线性 | bench 显示 O(n²) 超线性增长 | 复杂度异常 | 路径设计问题 |
| Phase 1: 2x 加速比 | Rust 远快于 Python，同类库达 10x+ | rs 仅 2x py | 与技术基本面冲突 | 不是极限，是架构问题 |
| Phase 2: 性能极限 | Phase 2 计算量 > Phase 1，Rust 优势被放大 | Phase 2 性能 < Phase 1 | 与递进逻辑矛盾 | 有结构性开销 |
| Phase 2.5: parse/build 比 | Python parse ≈ build（逻辑对称） | rs parse >> build | 对称性被破坏 | parse 有多余开销 |
| Phase 2.5: 差值 vs 字段数 | 固定开销不随字段数变化 | E1(2字段)与E3(6字段)差距相同 | 差值恒定 | 瓶颈是固定成本 |

每个案例用的逻辑约束不同。下一个问题会有新的约束来源。关键是先想清楚什么必须成立，再看数据是否满足。
