---
description: neoconstruct 项目对 AHE（Agentic Harness Engineering）通用工作流的项目特定补充规则。当 PM 执行 AHE iteration（生成 manifest / verify / harness 重组）时加载，作为 agentic-harness-engineering skill 的项目级附加约束。
---

# neoconstruct AHE Practices — 项目级 AHE 补充
...
> AHE 论文用 Terminal-Bench 类任务集；neoconstruct 的"测试任务集"= 真实开发任务。
...
| `neoconstruct-ahe-practices §W` | 项目级 AHE 补充（本 skill） | `neoconstruct-ahe-practices §D2` |
