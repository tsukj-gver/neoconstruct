---
id: neoconstruct-usage
name: neoconstruct 用法
description: 用 neoconstruct 定义和解析二进制协议。触发：实现二进制协议解析/构建时。
---

# neoconstruct 用法 SKILL

> 本 SKILL 自包含。读完本文件即可从零实现复杂二进制协议（条件分支 + bitfield + CRC），
> 不需要查阅任何其他文件。所有示例可直接复制运行。

## 1. neoconstruct 是什么（30 秒）

**neoconstruct** 是高性能二进制解析/构建库：**Rust 内核 + Python `@dataclass` API**。
...
**前置条件**：安装好 neoconstruct（`maturin develop --release`），Python 中 `from neoconstruct import ...` 可用。

## 2. 最小示例（2 分钟上手）

```python
from dataclasses import dataclass
from neoconstruct import StructMixin, field, Int8ub, Int16ub, Bytes
...
```python
from neoconstruct import StructMixin, BitStructMixin, field, rfield, wfield
...
> 本章是 SKILL 的核心：如何用 neoconstruct 表达真实协议结构。每个模式都有
> 完整可运行代码，复制即可上手。
...
```python
from dataclasses import dataclass
from neoconstruct import BitStructMixin, field, Bit, BitsInteger, Padding
...
```python
from neoconstruct import StructMixin, field, Int8ub, Int16ub, Switch
...
```python
from neoconstruct import StructMixin, field, rfield, Int8ub, Computed, Peek, Switch
...
```python
from neoconstruct import IfThenElse, Bytes
...
```python
from neoconstruct import Select, Int32ub, CString
...
```python
from neoconstruct import FocusedSeq, Const, Terminated, Bytes
...
neoconstruct 的 `Checksum` 有两条路径，**强烈推荐 StreamRange 模式**（路径 B）。
...
# 篡改字节 -> ChecksumError
from neoconstruct import ChecksumError
try:
...
```python
from neoconstruct import HashAlgo, Checksum, Bytes, rfield, Tell, field
...
```python
from neoconstruct import PrefixedArray, Byte, Int32ub
...
```python
from dataclasses import dataclass
from neoconstruct import StructMixin, field, Prefixed, Int16ub, GreedyBytes
...
```python
from neoconstruct import CString, GreedyString, PaddedString, PascalString
...
```python
from neoconstruct import Enum, Int8ub
...
```python
from neoconstruct import FlagsEnum, Int8ub
...
```python
from neoconstruct import Mapping, Int8ub
...
```python
from neoconstruct import StopIf, GreedyRange
...
```python
from neoconstruct import Rebuild, Default
...
```python
from dataclasses import dataclass
from neoconstruct import StructMixin, field, Int8ub, Int16ub
...
```python
from neoconstruct import Probe
...
- neoconstruct 的 `Sequence` build 不含 RO 字段占位（Python 原版需含 None 占位）
- 字段名引用语法不同：neoconstruct 用字段名直接引用，Python 原版用容器表达式语法（以原版官方文档为准）
...
> **本 SKILL 结束**。读完本文件 + 跑过本章示例，你已能用 neoconstruct 实现任何
