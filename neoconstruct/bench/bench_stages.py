...
- 子进程 A：sys.path 注入 neoconstruct/python，跑测量点 1（Rust parse）
...
def write_results_file(report: str, rs: dict, py: dict) -> Path:
    ...
    lines = []
    lines.append("=" * 78)
    lines.append("neoconstruct parse 内核分阶段计时（234ns/field 开销定位）")
    lines.append("=" * 78)
    lines.append("")
    lines.append("环境信息：")
...
def main() -> int:
    print("=" * 78)
    print("neoconstruct parse 内核分阶段计时（234ns/field 定位）")
    print("=" * 78)
    ...
    print("[1/2] 测量 neoconstruct + 纯 Python 操作（测量点 1-5）...")
    rs = _run_rs_stages()
