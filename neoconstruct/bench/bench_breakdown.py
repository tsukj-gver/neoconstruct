...
- 子进程 A：sys.path 注入 neoconstruct/python，测各阶段
...
_RS_SCRIPT = textwrap.dedent(
    ...
    from neoconstruct import StructMixin, field, Int32ub
...
_PY_SCRIPT = textwrap.dedent(
    ...
    # ---- sys.path 操纵：清除 neoconstruct，使用 site-packages 原版 ----
...
def write_results_file(report: str, results: list[dict]) -> Path:
    ...
    lines = []
    lines.append("=" * 72)
    lines.append("neoconstruct parse 分阶段计时（根因验证）")
    lines.append("=" * 72)
    lines.append("")
    lines.append("环境信息：")
...
def main() -> int:
    print("=" * 72)
    print("neoconstruct parse 分阶段计时（根因验证）")
    print("=" * 72)
    ...
    results = []
    for case, n in CASES:
        print(f"  测量 {case} (N={n}) neoconstruct 多阶段...", end="", flush=True)
        rs = _run_rs(n)
