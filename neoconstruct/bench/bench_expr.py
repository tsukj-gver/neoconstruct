"""neoconstruct vs Python construct 2.10.70 表达式场景性能基准测试。

3 个表达式用例（E1-E3），每个用例测量 parse 与 build 两个方向，对照：
- **neoconstruct**：本项目（Rust 内核 + Python 包层）
...
def format_results_table(results: dict) -> str:
    ...
    lines = []
    header = (
        f"{'用例':<4} {'方向':<6} {'neoconstruct (ns)':<18} "
...
def write_results_file(results: dict, table: str, failures: list[str]) -> Path:
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("neoconstruct vs Python construct 2.10.70 表达式场景性能测试结果")
    lines.append("=" * 70)
    lines.append("")
    lines.append("环境信息：")
...
def main():
    parser = argparse.ArgumentParser(
        description="neoconstruct 表达式场景性能基准测试"
    )
    parser.add_argument(
        "--case",
        action="append",
    ...
    print("=" * 70)
    print("neoconstruct vs Python construct 2.10.70 表达式场景性能测试")
    print("=" * 70)
