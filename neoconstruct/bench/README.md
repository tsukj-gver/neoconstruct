# neoconstruct Benchmark 套件

> 性能基准测试集合：neoconstruct vs Python construct 2.10.70 加速比测量。
...
所有 bench 脚本需在能访问两个实现（neoconstruct 与原版 construct）的环境运行：
- **neoconstruct**：通过 `neoconstruct/python/` 目录（sys.path 注入）
...
# 保留 sys.executable 逻辑，需主 python 装了两个实现）
...
2. 项目内 venv：`neoconstruct/.venv`（扩展）/ `neoconstruct/.venv-pc`（原版参考）
3. fallback `sys.executable`（最后手段）

bench_expr / bench_bitstream / bench_stages / bench_breakdown 4 个脚本保留
`sys.executable` 逻辑（要求主 python 同时可导入两个实现）。
