...
def AlignedStruct(modulus, **subconskw):
    ...
    在 neoconstruct 中，用户用 dataclass 语法（而非位置 subcons）。

    使用方式::

        >>> from neoconstruct import AlignedStruct, Int8ub, Int16ub
...
def Timestamp(subcon, unit, epoch):
    ...
    neoconstruct 实现为 Python 层（arrow 依赖，无法 Rust 化）。
    ...
    **subcon 类型检查**：若非描述符/Construct 实例则抛
    ...
    neoconstruct 提前在 macro 中检查，提供更友好的错误信息。
    ...
    if unit == "msdos" or epoch == "msdos":
        ...
        # neoconstruct 用 make_dataclass + BitStructMixin（与 AlignedStruct 同模式）
        ...
        # Container 是 dict 子类（neoconstruct 中用普通 dict 替代）
