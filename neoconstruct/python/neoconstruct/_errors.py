...
:class:`RepeatError`          ``ConstructError::Repeat``
:class:`StopFieldError`       ``ConstructError::StopField``
:class:`IndexFieldError`      ``ConstructError::IndexField``
:class:`StringError`          ``ConstructError::String``
:class:`ExplicitError`        ``ConstructError::Explicit``
:class:`SelectError`          ``ConstructError::Select``
===========================  ==========================================
...
class PaddingError(ConstructError):
    ...
    ``BitPaddingNode`` 在编译期对非法 pattern 返回此错误
...
class RepeatError(ConstructError):
    ...
    RepeatUntil build 遍历完列表无元素满足终止表达式时触发
    （core.py L2700）。
    """
...
class StringError(ConstructError):
    ...
    String 系列构造器（CString / GreedyString / PaddedString /
...
class SelectError(ConstructError):
    ...
    Select 构造器遍历全部 subcons 后无成功者时触发。
    """
...
class ConstError(ConstructError):
    ...
    ``Const`` 构造器的 parse/build 校验失败时触发
...
class TerminatedError(ConstructError):
    ...
    ``Terminated`` 构造器在 stream 仍有剩余字节时触发
    （core.py L4748）。
    """
...
class MappingError(ConstructError):
    ...
    - ``Mapping`` parse/build 时 key 不在映射（**包含 TypeError → MappingError 转换**：
      自定义不可哈希 key 的 TypeError 也会被捕获转换）
    """
...
class ValidationError(ConstructError):
    ...
    - ``OneOf`` parse/build 时 obj ∉ valids（含 bool 边界：``True == 1`` 视为 int 1）
...
class TimestampError(ConstructError):
    ...
    **仅 Python 层使用**（Timestamp macro 是 Python 实现，
...
def StringEncoded(subcon=None, encoding=None):  # noqa: D401 (函数名故意大写对齐 Python construct)
    ...
    neoconstruct 中，编解码逻辑已下沉到 :class:`CString` / :class:`GreedyString` /
    :class:`PaddedString` / :class:`PascalString` 内部。``StringEncoded`` 在
    Python construct 中是内部 Adapter（docstring 标注 "Used internally"），
    neoconstruct 不再需要它。

    :param subcon: 任意 Construct 实例（Python construct 兼容参数，neoconstruct 忽略）。
    :param encoding: 编码字符串（Python construct 兼容参数，neoconstruct 忽略）。
    ...
    """
    raise StringError(
        "StringEncoded is deprecated in neoconstruct; use CString / GreedyString / "
