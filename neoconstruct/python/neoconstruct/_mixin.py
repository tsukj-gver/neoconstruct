...
# 明确错误而非导入时失败。
try:
    from ._neoconstruct_core import compile_schema as _compile_schema
...
def _compile_schema_for_class(cls):
    ...
    if _compile_schema is None:
        raise ConstructError(
            "neoconstruct Rust 扩展不可用，无法编译 schema。"
...
class StructMixin:
    ...
    用法::
        ...
        from neoconstruct import StructMixin, field, rfield, Int8ub, GreedyBytes
...
class BitStructMixin(StructMixin):
    ...
    使用方式::
        ...
        from dataclasses import dataclass
        from neoconstruct import BitStructMixin, field, Bit, Nibble, BitsInteger
