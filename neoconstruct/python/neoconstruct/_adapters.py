...
class Adapter:
    ...
    对应 Python construct core.py:813 的 Adapter。在 neoconstruct 中，
    ...
    **双层分工**：Adapter/SymmetricAdapter 是用户面构造器，
    ...
    def __init__(self, subcon):
        ...
        :param subcon: 被包装的子构造器（任意 neoconstruct 描述符或 Adapter）。
        ...
        # neoconstruct 当前不使用此标志，保留以兼容用户面属性访问。
...
class Validator(SymmetricAdapter):
    ...
    neoconstruct 用户继承此类实现 ``_validate(self, obj, context, path) -> bool``。
    ...
    - **不导出 ExprValidator**：neoconstruct 不接收 lambda/callable。
