...
``_neoconstruct_core`` 扩展模块提供。本模块将其重导出，供 ``__init__.py``
和内部模块使用。
...
try:
    from ._neoconstruct_core import (
        # 描述符 pyclass
...
try:
    from ._neoconstruct_core import BytesDescriptor as Bytes
...
# Bitwise 描述符
#
# ``Bitwise(subcon)`` 是核心包装器，将字节流转为 bit 流。neoconstruct 通过
...
# construct 的 ``Array``。neoconstruct 通过 type name "ArrayDescriptor" 识别，
...
# construct 的 ``GreedyRange``。neoconstruct 通过 type name "GreedyRangeDescriptor"
...
# construct 的 ``PrefixedArray``。neoconstruct 通过 type name "PrefixedArrayDescriptor"
# 识别，构建 ``Node::PrefixedArray(PrefixedArrayNode)``。
#
# 与 Python 原版的关键差异：
# - neoconstruct 不依赖 FocusedSeq/Rebuild，而是独立 Node。
# - ``len_`` 辅助函数不实现。
# ---------------------------------------------------------------------------
...
class PrefixedArrayDescriptor:
    ...
    neoconstruct 用独立 Node 实现（不依赖 FocusedSeq/Rebuild）。
...
#   neoconstruct 中 IndexNode 是用户访问数组下标的唯一机制，
...
class PascalStringDescriptor:
    ...
    macro 嵌套；neoconstruct 独立实现，不依赖 Prefixed。
...
class SubconstructDescriptor:
    ...
    neoconstruct 把它实现为**具体节点**，主要供未来 Pointer/Prefixed 复用，
...
class RawCopyDescriptor:
    ...
    已知差异：neoconstruct 的 build 不返回值，
...
def Seek(at, whence=0):
    ...
    使用方式（在 Sequence 内）::
        ...
        from neoconstruct import Sequence, Seek, Bytes
...
def Pointer(offset, subcon, stream=None, relativeOffset=False):
    ...
    使用方式（正 offset 绝对定位）::
        ...
        from neoconstruct import Pointer, Bytes
        d = Pointer(8, Bytes(1))
...
def Prefixed(lengthfield, subcon, includelength=False):
    ...
    使用方式（基本）::
        ...
        from neoconstruct import Prefixed, Byte, Bytes
    ...
    使用方式（includelength=True）::
        ...
        from neoconstruct import Prefixed, Int16ub, GreedyBytes
        d = Prefixed(Int16ub, GreedyBytes, includelength=True)
        d.parse(b"\\x00\\x05abc")   # lengthfield=5，减 Int16ub sizeof=2 → 子流 3 字节

    使用方式（VarInt 前缀）::

        from neoconstruct import Prefixed, VarInt, GreedyRange, Int32ul
...
def Const(value, subcon=None):
    ...
    使用方式（bytes value，subcon 自动推断）::
        ...
        from neoconstruct import Const
    ...
    使用方式（int value，显式 subcon）::
        ...
        from neoconstruct import Const, Int32ub
...
def Default(subcon, value):
    ...
    使用方式（常量默认值）::
        ...
        from neoconstruct import Default, Byte
...
def Probe(into=None, lookahead=None):
    ...
    - Python ``Probe(lambda ctx: expr)`` 支持任意 callable。neoconstruct 仅支持
...
def Aligned(modulus, subcon, pattern=b"\x00"):
    ...
    使用方式（基本）::
        ...
        from neoconstruct import Aligned, Int16ub
...
# Hex / HexDump 描述符
#
# Hex/HexDump 是 construct 核心库内置 Adapter（非用户自定义），在 neoconstruct 中
...
def Hex(subcon):
    ...
    使用方式（int 字段）::
        ...
        from neoconstruct import Hex, Int32ub
    ...
    使用方式（bytes 字段）::
        ...
        from neoconstruct import Hex, Bytes
...
def HexDump(subcon):
    ...
    使用方式（bytes 字段）::
        ...
        from neoconstruct import HexDump, Bytes
...
def Checksum(checksumfield, hashfunc, bytesfunc=None, start=None, end=None):
    ...
    使用方式（Python callable + ContextBytes 兼容）::
        ...
        import hashlib
        from neoconstruct import RawCopy
...
class EnumDescriptor:
    ...
    ``Enum``（core.py L1920）。在 neoconstruct 中实现为 Rust Node（非 AdapterCallback）。
...
def Enum(subcon, *merge, **mapping):
    ...
    使用方式::
        ...
        from neoconstruct import Enum, Byte
...
def FlagsEnum(subcon, *merge, **flags):
    ...
    使用方式::
        ...
        from neoconstruct import FlagsEnum, Byte
...
def Mapping(subcon, mapping):
    ...
    使用方式::
        ...
        from neoconstruct import Mapping, Byte
...
def OneOf(subcon, valids):
    ...
    使用方式::
        ...
        from neoconstruct import OneOf, Byte
...
def NoneOf(subcon, invalids):
    ...
    使用方式::
        ...
        from neoconstruct import NoneOf, Byte
...
def Union(parsefrom, *subcons, **subconskw):
    ...
    使用方式::
        ...
        from neoconstruct import Union, Bytes, Int32ub
...
def Sequence(*subcons, **subconskw):
    ...
    使用方式::
        ...
        from neoconstruct import Sequence, Byte, Bytes, GreedyBytes
...
def ProcessXor(padfunc, subcon):
    ...
    使用方式::
        ...
        from neoconstruct import ProcessXor, Int16ub
...
def ProcessRotateLeft(amount, group, subcon):
    ...
    使用方式::
        ...
        from neoconstruct import ProcessRotateLeft, Int16ub
...
def NamedTuple(tuplename, tuplefields, subcon):
    ...
    使用方式（原版 construct 写法，Struct 作 subcon）::
    ...
    **已知 parity 差异**：NamedTuple over Struct 时，neoconstruct 只传
    tuplefields 命名的字段（忽略实例 __dict__ 中其他字段）；Python ``factory(**obj)``
    传 Container 所有字段，多余字段报 TypeError。neoconstruct 更宽松。
