"""neoconstruct 内部类（与 Python construct core.py 内部类对齐）。
...
这些类在编译期被 Rust 端 ``build_enum_node`` 加载（``py.import_bound("neoconstruct._internals")``），
...
class EnumIntegerString(str):
    ...
    编译期由 :class:`neoconstruct._descriptors.EnumDescriptor.__init__` 调用
    ...
    def __int__(self):
        ...
        ``int(str)`` 在非数字字符串上抛错的差异——neoconstruct 选择返回 0 而非
        抛错，避免 parse 路径意外失败）。
        """
