...
neoconstruct 用 Rust 重写了 Python `construct` 库的内核：通过 [pyo3](https://pyo3.rs) 直接操作 CPython C API，parse/build 各只有**一次 FFI 边界穿越**，不在 Rust 侧引入中间表示层。用户面是声明式的 `@dataclass` 语法，适合表达二进制协议、文件格式、网络报文。

## 安装

```bash
pip install neoconstruct
```
