"""用户报障场景回归测试：嵌套表达式 key 与值提供型 default 行为锁定 + 边界行为锁定用例。

期望值为行为语义锁定值；其中 Bitwise×Switch 与 Prefixed×Array(count) 两项
用当前库即可运行的「常量 key / 常量 count 对照组」锚定（对照组喂相同数据
实测，表达式变体应分派同分支同值——见各用例 docstring）。

覆盖：
- Prefixed × Switch 嵌套 key：parse/build/roundtrip（typ=1/2 两分支）
- 同族矩阵：PrefixedArray×Switch、Bitwise×Switch、Prefixed×If、
  Prefixed×Computed、Prefixed×Bytes(len)、Prefixed×Array(count)
- 值提供型 default：field(Const(...)) / field(Default(...)) 无参实例化 + build
- wfield(Padding(...)) 无参实例化 + build
- field(Rebuild(...)) 无参实例化 + build（值提供型同族泛化）
- 行为锁定：顶层 Switch(typ+1) parse/build（宽松行为，按功能保留）
- 负例锁定：Const(5) 无 subcon → CompilationError

dataclass 一律定义在测试函数体内：嵌套 key 类用例的失败点在类定义
...
# ---------------------------------------------------------------------------
# Prefixed × Switch 嵌套 key 回归
# ---------------------------------------------------------------------------


class TestPrefixedSwitchNestedKey:
    """Switch keyfunc 引用根字段，经 Prefixed 包装后正常编译 parse/build。
...
class TestC1WrapperConsumerMatrix:
    ...
    def test_prefixed_array_switch_build(self):
        """PrefixedArray×Switch build → ``b'\\x01\\x02\\x05\\x06'``。"""
    ...
    def test_bitwise_switch_parse(self):
        """Bitwise×Switch parse（typ=1 → Int8ub 分支）。
    ...
    def test_prefixed_if_parse_false_branch(self):
        """Prefixed×If parse，cond 为假 → br=None。"""
    ...
    def test_prefixed_computed_parse(self):
        """Prefixed×Computed parse → br=typ。"""
    ...
    def test_prefixed_computed_build(self):
        """Prefixed×Computed build → ``b'\\x01\\x00'``。
    ...
    def test_prefixed_bytes_len_build(self):
        """Prefixed×Bytes(typ) build → ``b'\\x02\\x02XY'``。"""
...
# ---------------------------------------------------------------------------
# 值提供型构造器隐式 default 推导
# ---------------------------------------------------------------------------


class TestValueProviderImplicitDefaults:
    """值提供型构造器作为 field/wfield 时无需强制实参。
    ...
    def test_const_field_no_arg_instantiation_and_build(self):
        """``field(Const(5, Int8ub))`` → ``PC()`` 可实例化，build 用常量 5。"""
    ...
    def test_default_field_no_arg_instantiation_and_build(self):
        """``field(Default(Int8ub, 9))`` → ``PD()`` 可实例化，build 用默认 9。"""
    ...
    def test_wfield_padding_no_arg_instantiation_and_build(self):
        """``wfield(Padding(2))`` → ``PW(n=1)`` 可实例化（值被忽略）。
    ...
    def test_rebuild_field_no_arg_instantiation_and_build(self):
        """``field(Rebuild(Int8ub, n))`` → ``PR(n=3)`` 可实例化。
...
class TestConstWithoutSubconNegative:
    ...
    非 bytes 且无 subcon 的 Const 无法确定编码字节，编译期即拒绝。
    """
