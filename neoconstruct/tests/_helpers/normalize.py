...
1. ``_`` 开头键过滤：Python construct Container 含 ``_io`` / ``_index`` 等内部状态，
   neoconstruct 用户面 dataclass 不暴露这些；过滤后两侧 dict 才能比对。
   neoconstruct dataclass 不允许 ``_`` 开头字段（PEP 8 + identifier 限制），故过滤无风险。
...
仅存在于测试子进程的 JSON 输出，用于跨 rs/py 实现比对；**不进入 neoconstruct 的
...
def normalize(value):
    """规范化输出值，便于跨实现（neoconstruct vs Python construct）比较。
