...
_TEST_DIR = Path(__file__).resolve().parent          # neoconstruct/tests/system
_PROJECT_ROOT = _TEST_DIR.parent.parent              # neoconstruct/
...
def _venv_python(venv_name: str) -> Path | None:
    ...
    venv_name 为 neoconstruct/ 下的 venv 目录名；未找到时返回 None
    （由调用方走后续 fallback）。
    """
...
_RS_PYTHON = _venv_python(".venv")     # neoconstruct 扩展
_PY_PYTHON = _venv_python(".venv-pc")  # 原版参考 construct==2.10.70


def get_rs_python() -> str:
    """CRS venv python.exe（含 neoconstruct wheel）。
    ...
    *,
    ...
    参数：
        rs_factory: 字符串形式的 Python 代码，定义名为 `make_protocol()` 的函数，
                    返回 neoconstruct 的 StructMixin 子类。字符串内含 import 等。
...
# For neoconstruct StructMixin, build is on instance:
