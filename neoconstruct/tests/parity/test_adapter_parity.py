...
差异处理：
- RC-build-1：RawCopy build 不返回带 data 的 Container（neoconstruct build 路径
...
# - Rebuild：Python construct 用 ``len_`` 上下文函数取 items 长度；neoconstruct 用
#   表达式（``items_count`` 字段引用）。两侧需用不同表达式但实现等价语义。
# - RawCopy build：Python 返回 Container(data=...)，neoconstruct 不返回。
#   parity 跳过 build 返回值对比，仅对比 build 字节输出。
# - AdapterCallback：Python 用 ``Adapter`` 基类 + _decode/_encode；neoconstruct 同。
_CASE_DEFINITIONS = '''
def _make_case_rs(case_id):
    ...
    if C == 'RB1':
        ...
        # neoconstruct：count = Rebuild(Int8ub, items_count 字段引用)
