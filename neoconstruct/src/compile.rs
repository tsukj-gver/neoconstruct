...
use crate::nodes::struct_node::{
    FieldMode, FieldName, InitDefault, StructField, StructNode, ValueKind,
};
...
pub fn compile_schema(
    ...
    for (i, (name, desc)) in field_names.iter().zip(descriptors.iter()).enumerate() {
        ...
        let expr_derived = expr_programs_slice.get(i).is_some_and(|p| p.is_some());
    ...
    let init_defaults: Vec<InitDefault> = fields.iter().map(|f| f.kind.init_default(py)).collect();
...
fn build_node_from_descriptor(
    ...
    match type_name
        ...
        // neoconstruct 新增（Python construct 无对应物）。Element 字段为 RepeatUntil
        ...
        // EnumInteger 类从 neoconstruct._internals 加载（fallback 路径用）。
        "EnumDescriptor" => {
    ...
    let focus_name: String = pbf_obj.extract().map_err(|_| ConstructError::Compilation {
        message: format!(
            "FocusedSeqDescriptor 'parsebuildfrom' must be a string (field index {}). \
                 neoconstruct only supports string parsebuildfrom (e.g. 'num'). Lambda \
...
/// # 已知差异
///
/// Python Rebuild.func 可以是任意 callable lambda。neoconstruct 收窄为仅表达式
            ...
            return Err(ConstructError::Compilation {
                message: format!(
                    "PointerDescriptor 'stream' parameter (stream switching) is not supported \
                     in neoconstruct (field index {}). Use the default stream (stream=None).",
                    field_index
                ),
            });
        }
...
///
/// 编译期分类：
/// - hashfunc.type() == "neoconstruct._hashalgo.HashAlgo" → BuiltinHash（按 .name 取值）
...
/// 与 `encmapping`（dict str→int）。`EnumInteger` 类从 `neoconstruct._internals` 加载。
fn build_enum_node(
    py: Python<'_>,
    desc: &Bound<'_, PyAny>,
    field_index: usize,
    ...
    // 加载 EnumInteger 类（从 neoconstruct._internals）。
    let enum_integer_cls = py
        ...
        .map_err(|e| ConstructError::Compilation {
            message: format!(
                "failed to load EnumInteger from neoconstruct._internals: {}",
                e
            ),
        })?;
    Ok(EnumNode::new(
