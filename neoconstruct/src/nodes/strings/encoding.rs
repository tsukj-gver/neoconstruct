...
/// 带"本机字节序 BOM"语义（不可移植），neoconstruct 强制用户显式指定字节序。
...
impl Encoding {
    ...
    /// neoconstruct 编译期拒绝，错误信息引导用户改用 `utf_16_le`/`utf_16_be`/
    /// `utf_32_le`/`utf_32_be`。
    ///
    /// # 错误引导文本
    ///
                ...
                Err(ConstructError::Compilation {
                    message: format!(
                        "encoding {:?} is ambiguous (Python adds native-endian BOM, \
                         which is non-portable). neoconstruct requires explicit byte order: \
                         use {:?} or {:?} instead.",
                        s, le_hint, be_hint,
                    ),
                })
