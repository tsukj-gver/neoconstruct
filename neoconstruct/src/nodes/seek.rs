...
//! Python construct 用 `flagbuildnone=True` 使 build 接受 None 输入；neoconstruct
...
///   （neoconstruct build 签名不返回值；Python `_build` 返回 stream_seek 返回值被
///   Sequence 忽略）
...
/// neoconstruct build 签名接收 `&Bound<PyAny>`，Seek build 忽略 obj 内容
...
mod tests {
    ...
    // ======================================================================
    // build — 接受 None（flagbuildnone 兼容）
    // ======================================================================
