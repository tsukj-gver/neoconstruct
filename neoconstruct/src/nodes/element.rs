...
//!
//! Python 参考无（neoconstruct 新增——Python construct 用 lambda 参数 `x`
//! 引用当前元素，neoconstruct 用 Element 字段 + 字段名引用机制）。
//!
//! ## 概述
//!
//! `Element` 是 neoconstruct 中 RepeatUntil 终止表达式引用"当前元素"的机制。
...
///
/// 对应 neoconstruct 用户面 `Element()`。Python construct 无对应物——Python 用
/// lambda 参数 `x` 引用当前元素，neoconstruct 用 Element 字段 + 字段名引用机制。
///
/// # parse 行为
///
