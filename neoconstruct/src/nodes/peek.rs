...
//! （不消费字节）。Python 用 try/except/finally 实现此语义；neoconstruct 用
//! `ParseStream::seek` + Result 匹配（Stream 抽象纯 Rust 内部）。
//!
//! ## ExplicitError 处理
//!
//! Python `Peek` 在 inner.parse 抛 `ExplicitError` 时**不吞掉**（向上传播）。
//! neoconstruct 以 `ConstructError::Explicit` 变体表达该错误，[`is_explicit_error`]
...
/// - build：no-op（对齐 Python `return obj`，但 neoconstruct build 不返回值）
/// - sizeof：返回 0
///
/// # ExplicitError 处理
///
